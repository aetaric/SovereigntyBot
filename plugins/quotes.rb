...
#  def setup(*)
#    $mongo[:quotes] = $mongo[:quotes]
#  end

  def quote(m,quote_num)
    rand = Kernel.rand()
    if !mod?(m)
      quote = $mongo[:quotes].find({ :channel => m.channel.name, :number => { '$gte' => rand }, :deleted => 0 },{:limit => 1}).first
      if quote.nil?
        quote = $mongo[:quotes].find({ :channel => m.channel.name, :number => { '$lte' => rand }, :deleted => 0 }, {:limit => 1}).first
      end
    else
      if quote_num.nil?
        quote = $mongo[:quotes].find({ :channel => m.channel.name, :number => { '$gte' => rand }, :deleted => 0 },{:limit => 1}).first
        ...
        quote = $mongo[:quotes].find({ :channel => m.channel.name, :number => quote_num, :deleted => 0 },{:limit => 1}).first
      end
    end
...
#      end
      ...
      count = $mongo[:quotes].count({ :channel => m.channel.name, :deleted => 0 }).to_i
      $mongo[:quotes].insert_one({ :date => local, :channel => m.channel.name, :quote => quote, :number => count, :deleted => 0 })
  ...
  def edit_quote(m,quote_num,quote)
    $mongo[:quotes].update_one({ :number => quote_num, :channel => m.channel.name, :deleted => 0 }, 
  ...
  def del_quote(m,quote_num)
    $mongo[:quotes].update_one({ :number => quote_num, :channel => m.channel.name }, { :deleted => 1 } )
